...
namespace iSpyPRO.DirectShow
{
    ...
    public class FilterInfo : IComparable
    {
        ...
        public int CompareTo(object value)
        {
            ...
            return f == null ? 1 : String.CompareOrdinal(Name, f.Name);
        }
        ...
        private static string GetName(IMoniker moniker)
        {
            ...
            try
            {
                ...
                // get it as string
                var ret = (string)val;
                return string.IsNullOrEmpty(ret) ? throw new ApplicationException() : ret;
            }
            catch (Exception)
            {
                return "";
        ...
        private string GetName(string monikerString)
        {
            ...
            // create bind context
            if (NativeMethods.CreateBindCtx(0, out IBindCtx bindCtx) == 0)
            {
                // convert moniker`s string to a moniker
                if (NativeMethods.MkParseDisplayName(bindCtx, monikerString, ref n, out IMoniker moniker) == 0)
                {
                    // get device name
                    name = GetName(moniker);
