...
namespace iSpyApplication
{
    public partial class AddCamera : Form
    {
        ...
        {
            if (CameraControl.Camera?.MotionDetector != null)
            {
        ...
        private bool SelectSource()
        {
            ...
            if (vs.DialogResult == DialogResult.OK)
            {
                ...
                if (CameraControl.VolumeControl == null && CameraControl.Camera != null)
                {
                    ...
                    if (CameraControl.Camera.VideoSource is ISupportsAudio c)
                    {
        ...
        {
            int j;
            if (CameraControl.Camobject.id == -1 && !SelectSource())
            {
                _forceClose = true;
                Close();
                return;
            ...
            {
                if (string.Equals(o.ToString(), CameraControl.Camobject.settings.cloudprovider.provider, StringComparison.OrdinalIgnoreCase))
                {
        ...
        private void ActionEditor1LoginRequested(object sender, EventArgs e) => Login();
        ...
        private void LoadAlertTypes()
        {
            ...
            if (CameraControl.Camobject.alerts.mode != null)
            {
                ...
                {
                    if (string.Equals(name, CameraControl.Camobject.alerts.mode, StringComparison.OrdinalIgnoreCase))
                    {
                        iMode = iCount;
                        break;
                    }
        ...
        private void CameraNewFrame(object sender, NewFrameEventArgs e)
        {
            ...
            try
            {
                ...
                if (_ctw?.TripWireEditor1 != null)
                {
                    _ctw.TripWireEditor1.LastFrame = e.Frame;
                }
                if (_pip?.areaSelector1 != null)
                {
        ...
        private void BtnNextClick(object sender, EventArgs e) => GoNext();

        private void GoNext() => tcCamera.SelectedIndex++;

        private void GoPrevious() => tcCamera.SelectedIndex--;
        ...
        private bool CheckStep1()
        {
            ...
            if (MainForm.Cameras.FirstOrDefault(p => string.Equals(p.name, name, StringComparison.OrdinalIgnoreCase) && p.id != CameraControl.Camobject.id) != null)
        ...
        private bool Save()
        {
            ...
            if (!String.IsNullOrEmpty(audioip) && !IPAddress.TryParse(audioip, out IPAddress aip))
            {
                try
                {
            ...
            int.TryParse(txtBuffer.Text, out int bufferseconds);
            int.TryParse(txtCalibrationDelay.Text, out int calibrationdelay);
            int.TryParse(txtInactiveRecord.Text, out int inactiveRecord);
            int.TryParse(txtMaxRecordTime.Text, out int maxrecord);
            ...
            if (CameraControl != null && CameraControl.Camera?.VideoSource != null && CameraControl.Camera.VideoSource is VideoCaptureDevice vcd && vcd.SupportsProperties)
            {
                //save extended properties of local device

                vcd.GetProperty(VideoProcAmpProperty.Brightness, out int b, out VideoProcAmpFlags fb);
                vcd.GetProperty(VideoProcAmpProperty.Contrast, out int c, out VideoProcAmpFlags fc);
                vcd.GetProperty(VideoProcAmpProperty.Hue, out int h, out VideoProcAmpFlags fh);
                vcd.GetProperty(VideoProcAmpProperty.Saturation, out int s, out VideoProcAmpFlags fs);
                vcd.GetProperty(VideoProcAmpProperty.Sharpness, out int sh, out VideoProcAmpFlags fsh);
                vcd.GetProperty(VideoProcAmpProperty.Gamma, out int gam, out VideoProcAmpFlags fgam);
                vcd.GetProperty(VideoProcAmpProperty.ColorEnable, out int ce, out VideoProcAmpFlags fce);
                vcd.GetProperty(VideoProcAmpProperty.WhiteBalance, out int wb, out VideoProcAmpFlags fwb);
                vcd.GetProperty(VideoProcAmpProperty.BacklightCompensation, out int bc, out VideoProcAmpFlags fbc);
                vcd.GetProperty(VideoProcAmpProperty.Gain, out int g, out VideoProcAmpFlags fg);
            ...
            CameraControl.Camobject.settings.cloudprovider.provider = ddlCloudProviders.SelectedIndex > 0 ? ddlCloudProviders.SelectedItem.ToString() : "";
        ...
        private void ChkScheduleCheckedChanged(object sender, EventArgs e) => scheduleEditor1.Enabled = chkSchedule.Checked;

        private void TxtCameraNameKeyUp(object sender, KeyEventArgs e) => CameraControl.Camobject.name = txtCameraName.Text;
        ...
        private void TxtCameraNameTextChanged(object sender, EventArgs e) => CameraControl.Camobject.name = txtCameraName.Text;
        ...
        {
            if (CameraControl.Camera?.VideoSource != null && CameraControl.Camobject.settings.suppressnoise != chkSuppressNoise.Checked)
            {
        ...
        private void Button2Click(object sender, EventArgs e) => GoPrevious();
        ...
        {
            if (CameraControl.Camera?.VideoSource != null &&
                CameraControl.Camera.MotionDetector != null && (string)_processortypes[ddlProcessor.SelectedIndex] != CameraControl.Camobject.detector.postprocessor)
            {
                CameraControl.Camobject.detector.postprocessor = (string)_processortypes[ddlProcessor.SelectedIndex];
        ...
        private void LinkLabel1LinkClicked1(object sender, LinkLabelLinkClickedEventArgs e) => MainForm.OpenUrl(MainForm.Website + "/userguide-motion-detection.aspx#2");
        ...
        private void LinkLabel2LinkClicked(object sender, LinkLabelLinkClickedEventArgs e) => MainForm.OpenUrl(MainForm.Website + "/userguide-ftp.aspx");


        private void Login() => MainClass.Connect(MainForm.Website + "/subscribe.aspx", false);

        private void PnlPtzMouseDown(object sender, MouseEventArgs e) => ProcessPtzInput(e.Location);
        ...
        {
            if (ddlPTZ.SelectedItem is ListItem li)
            {
        ...
        private void PnlPtzMouseUp(object sender, MouseEventArgs e) => CameraControl.PTZ.CheckSendStop();
        ...
        private void DdlTimestampKeyUp(object sender, KeyEventArgs e) => CameraControl.Camobject.settings.timestampformatter = ddlTimestamp.Text;
        ...
        private void LinkLabel8LinkClicked(object sender, LinkLabelLinkClickedEventArgs e) => MainForm.OpenUrl(MainForm.Website + "/userguide-pairing.aspx");

        private void RdoFtpIntervalCheckedChanged(object sender, EventArgs e) => txtUploadEvery.Enabled = rdoFTPInterval.Checked;
        ...
        private void DdlAlertModeSelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (last != ddlAlertMode.SelectedItem.ToString() && CameraControl.Camera?.Plugin != null)
            {
        ...
        private void ChkTimelapseCheckedChanged(object sender, EventArgs e) => groupBox1.Enabled = chkTimelapse.Checked;

        private void chkPublic_CheckedChanged(object sender, EventArgs e)
        {
        }

        private void LinkLabel9LinkClicked(object sender, LinkLabelLinkClickedEventArgs e) => Login();
        ...
        private void numMaxFR_ValueChanged(object sender, EventArgs e) => CameraControl.Camobject.settings.maxframerate = numMaxFR.Value;

        private void button4_Click(object sender, EventArgs e)
        {
            var cp = new ConfigureProcessor(CameraControl);
            if (cp.ShowDialog(this) == DialogResult.OK && CameraControl.Camera?.MotionDetector != null)
            {
                CameraControl.SetDetector();
            }
            cp.Dispose();
        ...
        private void button5_Click(object sender, EventArgs e) => ConfigFilter();

        private void chkColourProcessing_CheckedChanged(object sender, EventArgs e)
        {
            if (chkColourProcessing.Checked && String.IsNullOrEmpty(CameraControl.Camobject.detector.colourprocessing))
            {
                if (!ConfigFilter())
        ...
        private void AddCamera_HelpButtonClicked(object sender, System.ComponentModel.CancelEventArgs e) => MainForm.OpenUrl(MainForm.Website + "/userguide-camera-settings.aspx");
        ...
        private void rdoContinuous_CheckedChanged(object sender, EventArgs e) => CameraControl.Camobject.alerts.processmode = "continuous";
        ...
        private void llblEditPTZ_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e) => Process.Start("Notepad.exe", Program.AppDataPath + @"XML\PTZ2.xml");
        ...
        private void btnAdvanced_Click(object sender, EventArgs e)
        {
            try
            {
                if (CameraControl.Camera.VideoSource is VideoCaptureDevice vcd)
                {
        ...
        private void ddlTimestamp_SelectedIndexChanged(object sender, EventArgs e) => CameraControl.Camobject.settings.timestampformatter = ddlTimestamp.Text;

        private void chkLocalSaving_CheckedChanged(object sender, EventArgs e) => gbLocal.Enabled = chkLocalSaving.Checked;
        ...
        private void linkLabel14_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e) => MainForm.OpenUrl(MainForm.Website + "/userguide-grant-access.aspx");
        ...
        {
            if (chkIgnoreAudio.Checked && CameraControl.VolumeControl != null)
            {
                MainClass.RemoveMicrophone(CameraControl.VolumeControl, false);
            }
        }
        ...
        private void btnAddPreset_Click(object sender, EventArgs e)
        {
            ...
            {
                var s = p.Val.Trim();
                if (!String.IsNullOrEmpty(s) && CameraControl.PTZ != null)
                {
                    try
                    {
        ...
        private void btnDeletePreset_Click(object sender, EventArgs e)
        {
            ...
            if (p != null && CameraControl.PTZ != null)
            {
                var li = (ListItem)p;
                try
                {
        ...
        private void chkStorageManagement_CheckedChanged(object sender, EventArgs e) => tblStorage.Enabled = chkStorageManagement.Checked;
        ...
        private void rdoMotion_CheckedChanged(object sender, EventArgs e) => CameraControl.Camobject.alerts.processmode = "motion";
        ...
        private void ddlRotateFlip_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (changed)
            {
                Enum.TryParse(CameraControl.Camobject.rotateMode, out RotateFlipType rmold);
                Enum.TryParse(li.Value, out RotateFlipType rmnew);
        ...
        private void rdoSaveInterval_CheckedChanged(object sender, EventArgs e) => numSaveInterval.Enabled = rdoSaveInterval.Checked;
        ...
        private void btnAuthorise_Click(object sender, EventArgs e) => Authorise(ddlCloudProviders.SelectedItem.ToString().ToLower());

        private void ddlCloudProviders_SelectedIndexChanged(object sender, EventArgs e) => btnAuthorise.Enabled = ddlCloudProviders.SelectedIndex > 0;
        ...
        private void btnAuthoriseYouTube_Click(object sender, EventArgs e) => Authorise("youtube");
        ...
        private void numProcessInterval_ValueChanged(object sender, EventArgs e) => CameraControl.Camobject.detector.processframeinterval = (int)numProcessInterval.Value;

        private void rdoTrigger_CheckedChanged(object sender, EventArgs e) => CameraControl.Camobject.alerts.processmode = "trigger";
